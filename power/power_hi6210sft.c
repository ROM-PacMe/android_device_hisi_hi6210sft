...
static int boostpulse_open(struct hi6210sft_power_module *hi6210sft)
{
    char buf[80];
    int len;

    if (hi6210sft->boostpulse_fd < 0) {
        hi6210sft->boostpulse_fd = open(BOOSTPULSE_PATH, O_WRONLY);
...
{
    struct hi6210sft_power_module *hi6210sft =
            (struct hi6210sft_power_module *) module;
    switch (feature) {
    default:
                                ...
                                void *data)
{
    ...
    switch (hint) {
     case POWER_HINT_INTERACTION:
        if (boostpulse_open(hi6210sft) >= 0) {
